...
$cat_title = $db->VeriOkuTek($vt2, "name", "id", $cat_id);

$new_link = $link . 'subpage=items&cat_id=' . $cat_id . "&";
...
foreach($tablo as $sira => $rows){
	if($rows["type"] == "image" || $rows["type"] == "file") {
		$arr = $rows["options"];
...
header("Location: ". $new_link . "alert=1");
} elseif($action == "add") {
if($_POST) {
$vt_row = array("NULL", "?");
$vt_value = array($cat_id);
...
$new_name = seo($handle->file_src_name);
$ad=$new_name . "doc" . rand(0,9999). rand(0,9999);
$handle->file_new_name_body = $ad;
...
$new_name = seo($handle->file_src_name);
		...
		$vt_row[] = "?";
		$vt_value[] = seo($_POST[$rows["options"]]);
	} else {
...
$verot_alert = 0;
	...
	$handle = new Upload($_FILES[$rows["url"]], 'tr_TR');
		...
		if($handle->processed) {
			$db->veriGuncelle($vt,array($rows["url"]),array($ad . "." . "pdf",$id),"id");
...
$new_name = seo($handle->file_src_name);
		...
		$db->veriGuncelle($vt,array($rows["url"]),array(seo($_POST[$rows["options"]]),$id),"id");
	}  else {

	}
}
}
	header("Location: ". $new_link . "alert=2");
}
}
...
?>
	...
	<link href="assets/plugins/bootstrap-daterangepicker/daterangepicker.css" rel="stylesheet">
			...
			<link rel="stylesheet" href="assets/plugins/magnific-popup/dist/magnific-popup.css"/>
										...
										<div class="row">
													<div class="col-sm-12">
															<h4 class="page-title"><?=$cat_title;?></h4>
															<ol class="breadcrumb">
																	<li><a href="index.php">Ana Sayfa</a></li>
								<?php if($action == "add") {
									echo '<li><a href="' . $link . '">' . $page_name . '</a></li>';
									echo '<li><a href="' . $new_link . '">' . $db->VeriOkuTek("products_category", "isim", "id", $db->VeriOkuTek("products_items", "cat_id", "id", $_GET["id"])) . '</a></li>';
									...
									echo '<li><a href="' . $link . '">' . $page_name . '</a></li>';
									echo '<li class="active">' . $cat_title . '</li>';
								}
								?>
...
if($rows["type"] == "text") {
echo '<div class="form-group"><label class="col-md-2 control-label">' . $rows["name"] . '</label><div class="col-md-7"><input ' . $zorunlu . ' type="text" name="' . $rows["url"] . '" class="form-control"></div></div>'; }
elseif($rows["type"] == "hidden") {  }
elseif($rows["type"] == "textarea") { echo '<div class="form-group"><label class="col-md-2 control-label">' . $rows["name"] . '</label><div class="col-md-7"><textarea ' . $zorunlu . ' name="' . $rows["url"] . ' id="' . $rows["url"] . '" class="form-control" rows="5"></textarea></div></div>'; }
...
<?php } elseif($action == "edit") { ?>
												<div class="col-lg-12">
													<div class="card-box">
														...
														<p class="text-muted m-b-30 font-13">
									* Zorunlu alanlar <br>
									<b>** Eğer PDF Dosyalarını değiştirecekseniz lütfen PDF Dosyasının yanında RESİMi de tekrar yükleyiniz.</b>
								</p>
								<?php
									if($verot_alert == 2) { alert_error("Yüklenirken bir hata oluştu (HATA 1)"); }
									elseif($verot_alert == 3) { alert_error("Yüklenirken bir hata oluştu (HATA 1)"); }
								?>
														<div class="row">
															<div class="col-md-12">
																<form class="form-horizontal" action="<?=$new_link;?>action=edit&id=<?=$id;?>" method="post" enctype="multipart/form-data" role="form">
										<input type="hidden" name="rand" value="<?=rand();?>"/>
<?php
foreach($tablo as $sira => $rows){
if($rows["add"] == 1) {
...
if($db->VeriOkuTek($vt, $rows["url"], "id", $id) != "") {
	echo '<div class="col-md-3"><a target="_blank" href="../upload/'.$db->VeriOkuTek($vt, $rows["url"], "id", $id).'" class="btn btn-default waves-effect waves-light"> <i class="fa fa-download m-r-5"></i> <span>Şuanki Dosyayı Göster</span> </a>';
	echo' &nbsp<a href="verisil.php?name='. $rows["url"].'&current='.$_SERVER["REQUEST_URI"].'" class="btn btn-default waves-effect waves-light"> <i class="fa fa-download m-r-5"></i> <span>Dosyayı Sil</span> </a></div>';
...
?>
												<div class="col-lg-12">
													<div class="card-box">
														<div class="row">
															<div class="col-sm-8">
																<form role="form" action="<?=$new_link;?>" method="post">
															...
															<div class="col-sm-4">
																 <a href="<?=$new_link;?>action=add" class="btn btn-default btn-md waves-effect waves-light m-b-30"><i class="md md-add"></i> Yeni</a>
															</div>
														</div>
<div id="sortable_sonuc"></div>
<?php
if($_POST["search"] == "") {
$search = " WHERE cat_id = '" . $cat_id . "' ";
} else {
$search = " WHERE cat_id = '" . $cat_id . "' and ";
foreach($tablo as $sira => $rows){
	...
	if(in_array($row["type"], $normal_kayit)) {
		...
		if($_POST["search"] == "") {
			...
			echo '<td style="width:20px"><i class="glyphicon glyphicon-sort"></i></td>';
												...
												<div class="btn-group">
																										<a href="<?=$new_link;?>action=edit&id=<?=$rows->id;?>" title="Düzenle" class="btn btn-icon btn-warning waves-effect waves-light"><i class="md md-edit"></i></a>
																										<a href="javascript:;" onclick="$.Notification.confirm('black','top center', 'Silmeye hazır!', '<?=$new_link;?>action=delete&id=<?=$rows->id;?>')" title="Sil" class="btn btn-icon btn-danger  waves-effect waves-light"><i class="md md-close"></i></a>
												</div>
																									</td>
																							</tr>
<?php } ?>
...
function disyap(ad){
	if(document.getElementById(ad).disabled == true) document.getElementById(ad).disabled = false;
	else document.getElementById(ad).disabled = true;
}
</script>
