...
class dbcon{
		...
		public $veritabani = "kilavuzegitim2_db"; //veritabanı adı
		public $host="localhost";
		public $hesap="kilavuzegit2_db"; //kullanıcı adı
		public $sifre="EmpeRiaN2"; //sifre
		public $baglan;
	...
	function VeriOkuRastgele($tablo,$alan,$istenmeyen = false){
		...
		function veriAra($veri = array(),$field = array(),$kelime){
			$sonuc = array();
			foreach($veri as $item){
				foreach($item as $key => $value){
					foreach($field as $sutun){
						if($sutun == $key){
							if(strpos(strtolower($value), strtolower($kelime)) !== false){
								$sonuc[] = $item;
							}
						}
					}
